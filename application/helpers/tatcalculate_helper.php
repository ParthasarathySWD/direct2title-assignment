    ...
    if($cust_delaytime!=0)
    {
      ...
      foreach ($cust_delaytime as $value) 
      {
        ...
        if($value->CustomerDelayStopTime=='0000-00-00 00:00:00')
          ...
          $cdelay_time[] = working_hour($start,$delay_end,$holidays);   
        } 
...
function working_hour($start,$end,$holidays)
{
...
function Get_CustomerDelayTime_ByOrder($OrderUID)
{
  ...
  $query = $CI->db->query("SELECT OrderUID,CustomerDelayStartTime, CustomerDelayStopTime FROM mcustomerdelay WHERE OrderUID ='$OrderUID' AND CustomerDelayStopTime = '0000-00-00 00:00:00'");
  if($query->num_rows()>0)
  {
    return $query->result();
  } else {
