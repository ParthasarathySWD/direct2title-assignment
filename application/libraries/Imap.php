...
class Imap
{
	 ...
	 * @return boolean
	 */
	public function message_setflag($uids, string $flag)
	{
		if (is_array($uids))
		{
			$uids = implode(',', $uids);
	 ...
	 * @return boolean
	 */
	public function message_clearflag($uids, string $flag)
	{
		if (is_array($uids))
		{
			$uids = implode(',', $uids);
	 ...
	 * @return string
	 */
	protected function get_html_custom(int $uid, $mimetype = '', $structure = false, $part_number = '')
	{
		if (! $structure)
		{
			$structure = imap_fetchstructure($this->stream, $uid, FT_UID);
		...
		if ($structure)
		{
				$text = imap_fetchbody($this->stream, $uid, 1.2, FT_UID | FT_PEEK);
