...
$("[data-toggle=reverse_workflowpopover]").on('hidden.bs.popover', function () {
  $('.selectpopover').select2('destroy');
});
...
$(document).on('submit','#onholdform',function(event){
  ...
  if(remarkstext == '' || menu_url == '' || OrderUID == ''){
    $.gritter.add({
    ...
    data: $('#onholdform').serialize() + "&" + $.param({'MenuUrl': menu_url})+ "&" + $.param({"remarkstext": remarkstext}) + "&" + $.param({'OrderUID': OrderUID})+ "&" + $.param({"reason": reason}) +"&" + $.param({"Type": Type}),
      dataType:'json',
  ...
  $('.Reviewunholdorder').click(function(){
    var MenuUrl = menuurl();
  ...
  $(document).off('submit', '#frmreverseworkflowpopup').on('submit', '#frmreverseworkflowpopup', function(e){
    e.preventDefault();
    e.stopPropagation();

    var button = $('#btn-reverse');
    ...
    $.ajax({
      type: "POST",
      url: CONST_BASEURL+'order_complete/ReverseWorkflow/',
      dataType:'json',
      data:formdata,
      processData: false,
      contentType: false,
  ...
  $(document).on('submit','#reviewonholdform',function(event){
    event.preventDefault();
    var OrderUID = $(".OrderUID").val();
    ...
    success: function(data)
    {
      ...
      if(data['Error'] == '0'){
        $.gritter.add({
          ...
          speed:'fast',
          after_close: function(){
            //window.location.replace('http://localhost/d2t.development/my_orders');
            window.location.reload();
         }
       });
...
$('.unholdorder').click(function(){
    ...
    $.ajax({
     ...
     data:{'MenuUrl':menu_url,'OrderUID':OrderUID},
     success: function(data)
     {
      button.attr('disabled',true);
      button.html('<i class="fa fa-spinner fa-spin"></i>');
 ...
 $(document).on('submit','#Frm_ExceptionRaise',function(event)
 {
  var MenuUrl = menuurl();
  var base_url = baseurl();  
  event.preventDefault();
 ...
 $(document).on('submit','#frmrevokeworkflow',function(event)
 {
    event.preventDefault();
    var base_url = baseurl();
...
$(document).on('submit','#qcform',function(event)
{
	event.preventDefault();
	var button = $('.qcsubmit');
	var OrderUID = $('.OrderUID').val();
				...
				success: function(data)
				{
					...
					if(data['Error'] == '0'){
						$.gritter.add({
							...
							speed:'fast',
							after_close: function(){
								window.location.replace(base_url+'my_orders');
							}
						}); 
					}else{ 
						button.removeAttr('disabled');
...
$(document).on("click",".qc_checkbox",function(){    
...
$(document).on('click',  '.BillComplete_Order', function (event) {
	event.preventDefault();
	button = $(this);
	button_text = $(this).html();
	var refresh = $(this).attr('data-refresh');
	var OrderUID = $('#OrderUID').val();
	if(OrderUID ==  '' || typeof OrderUID === "undefined")
	{
		$.gritter.add({title: '<i class="fa fa-bell"></i> Failed!', class_name: 'color danger',fade: true,time: 3000,speed:'fast'});
		return false;
	}
	...
	$.ajax({
		type: "POST",
		url: base_url + 'order_complete/BillComplete_Order',
		data: {'OrderUID':OrderUID},
		dataType:'json',
		beforeSend: function(){
			button.attr("disabled", true);
		...
		{ 
			...
			if (data.success == 1) 
			{
				...
				{
					button.html('<i class="fa fa-spinner fa-spin"></i> Redirecting ...'); 
				}
				$.gritter.add({
					title: '<i class="fa fa-bell"></i> '+data['message'], class_name: 'color success',fade: true,time: 1000,speed:'fast',
					after_close: function(){
						if(refresh == '1')
						{
							window.location.reload();
						...
						{
							window.location.replace(base_url+'myorders');
						}
					}
				});

			} else {
				$.gritter.add({title: '<i class="fa fa-bell"></i> '+data['message'], class_name: 'color danger',fade: true,time: 3000,speed:'fast'});
				button.html(button_text);
				button.removeAttr("disabled");
